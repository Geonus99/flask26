...
# 라우터(경로), get, post, 세션, 시크릿키
app = Flask(__name__)
app.secret_key = 'your_secret_key_here'

@app.route('/login', methods=['GET','POST']) # http://localhost:5000/lgoin
    # methods는 웹에 동작을 관여한다.
    # GET : URL 주소로 데이터를 처리(보안상 좋지 않지만 빠름)
    # POST : BODY영역에서 데이터를 처리(보안상 좋음, 대용량에서 많이 사용함)
    # 대부분 처음에 화면(HTML랜더)을 요청할 때는 GET 방식으로 처리
    # 화면에 있는 내용을 백엔드로 전달할 때는 POST 방식으로 처리
def login():
    if request.method == 'GET': # 처음접속하면 GET방식으로 화면 출력
        return render_template('login.html')
        # get방식으로 요청하면 login.html 화면이 나옴

    # login.html에서 action="/login" method="POST" 처리용 코드
    # login.html에서 넘어온 form 데이터 uid / upw
    ...
    # 2. 사용자가 '등록하기' 버튼을 눌러서 데이터를 보냈을 때 (DB 저장)
        ...
        try:
            with conn.cursor() as cursor:
                ...
                conn.commit()
            return redirect(url_for('board_list')) # 저장 후 목록으로 이동 redirect : 값 없이 호출할때(get방식)
        except Exception as e:
            print(f"글쓰기 에러: {e}")
...
def board_list():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            return render_template('board_list.html', boards=boards) # 파일로 값 보낼때 씀
    finally:
        conn.close()
...
@app.route('/') # url 생성용 코드 http://localhost:5000/ or http://내ip(192.168.0.???):5000
def index():
